	...
	protected void paintComponent(Graphics g) {
		...
		for (int i = 0; i < Gui.achievementSlider.length; i++) {
			if (Gui.achievementSlider[i].isVisible()) {
				g.setColor(Color.WHITE);
				g.drawRect(Gui.achievementSlider[i].getX(), Gui.achievementSlider[i].getY(), Gui.achievementSlider[i].getWidth(), Gui.achievementSlider[i].getHeight());
				g.drawImage(Gui.achievementSlider[i].getImage(), Gui.achievementSlider[i].getX() + 5, Gui.achievementSlider[i].getY() + 5, 40, 40, null);
				g.setFont(new Font("Arial", Font.PLAIN, 12)); g.drawString(Gui.achievementSlider[i].getText(), Gui.achievementSlider[i].getX() + 50,
