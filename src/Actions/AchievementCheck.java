			...
			public void run() {
				...
				if (!Gui.achievement[3].isUnlocked()) {
					...
					for (int i = 0; i < Gui.upgrade.length; i++) {
						if (Gui.upgrade[i].getAnzahl() > 0) {
							temp++;
						}
					}
					
					if (temp == Main.ugAnzahl) {
				...
				if (!Gui.achievement[5].isUnlocked()) {
					...
					if (Gui.upgrade[0].getAnzahl() >= 100) {
